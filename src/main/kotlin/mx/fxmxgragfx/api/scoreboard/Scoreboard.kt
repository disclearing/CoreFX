...
class Scoreboard(private val player: Player) {
    ...
    init {
        val board = Core.getInstance().server.scoreboardManager.newScoreboard

        objective = board.registerNewObjective("cougar", "dummy")
    ...
    fun update() {
        val configuration = Core.getInstance().scoreboardEngine.configuration!!
    ...
    private fun separate(line: String, usedBaseScores: MutableCollection<String>): Array<String> {
        ...
        when (this.separateScores.size) {
            ...
            3 -> {
                ...
                Core.getInstance().logger.warning("Failed to separate scoreboard line. Input: $line")
            }
        }

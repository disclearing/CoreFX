package mx.fxmxgragfx.api.command.bukkit

import mx.fxmxgragfx.api.async.Async
import mx.fxmxgragfx.api.command.CommandCompat
import mx.fxmxgragfx.api.command.CommandNode
import mx.fxmxgragfx.api.command.data.argument.ArgumentProcessor
...
class ExtendedCommand(val node: CommandNode, private val plugin: JavaPlugin) : Command(node.name, "", "/", node.getRealAliases().toList()), PluginIdentifiableCommand {
    ...
    override fun execute(sender: CommandSender, label: String, args: Array<String>): Boolean {
        val label = label.replace("${plugin.name.lowercase(Locale.getDefault())}:", "")
            ...
            if (executionNode.async) {
                ...
                Async {
                    try {
                        ...
                        executionNode.getUsage(realLabel).send(sender)
                        sender.sendMessage("${ChatColor.RED}An error occurred while processing your api.command.")
    ...
    fun tabComplete(sender: CommandSender, cmdLine: String): List<String> {
        ...
        val rawArgs = cmdLine.replace("${plugin.name.lowercase(Locale.getDefault())}:", "").split(" ")
        ...
        if (rawArgs.isEmpty()) {
            ...
            try {
                ...
                val fixed = 0.coerceAtLeast(currentIndex - 1)
                ...
                val parameterType: ParameterType<*>? = Core.getInstance().commandHandler.getParameterType(data.type)
    ...
    private fun concat(label: String, args: Array<String>): Array<String> {
        ...
        list.addAll(args.toList().stream().filter { it != null }.collect(Collectors.toList()))

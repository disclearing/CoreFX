...
import mx.fxmxgragfx.api.command.data.argument.Arguments
...
@Suppress("unused", "ReplaceIsEmptyWithIfEmpty", "MemberVisibilityCanBePrivate")
class CommandNode(var name: String?,
    ...
    private var owningClass: Class<*>? = null
    ...
    fun registerCommand(commandNode: CommandNode) {
        if (commandNode.name != null) {
            ...
            throw IllegalArgumentException("Cannot register api.command without a name to root node")
        }
    }

    fun hasCommand(name: String): Boolean = children.containsKey(name.lowercase(Locale.getDefault()))
    fun getCommand(name: String): CommandNode? = children[name.lowercase(Locale.getDefault())]
    ...
    fun isValidFlag(test: String): Boolean {
        return if (test.length == 1) {
            ...
            validFlags.contains(test.lowercase(Locale.getDefault()))
        }
    }
    ...
    operator fun invoke(sender: CommandSender, arguments: Arguments): Boolean {
            ...
            if (hasCommands()) {
                if (getSubCommands(sender, true).isEmpty()) {
                    if (hidden) {
                        ...
                        var noPermissionMessage = "${ChatColor.RED}You don't have permission to use this api.command."
        ...
        for (data in this.parameters) {
            if (data is FlagData) {
                ...
                var type: ParameterType<*>? = Core.getInstance().commandHandler.getParameterType(data.type)
        ...
        try {
            ...
            val executionThreshold = 10 // api.command threshold

            if (!async && logToConsole && stop - start >= executionThreshold) {
                Core.getInstance().logger.warning("Command '/" + getFullLabel() + "' took " + (stop - start) + "ms!")
            }

            return true
        } catch (e1: IllegalAccessException) {
            throw CommandException("An error occurred while executing the api.command", e1)
        } catch (e2: InvocationTargetException) {
            throw CommandException("An error occurred while executing the api.command", e2)
        }
    }
